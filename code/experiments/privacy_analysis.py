"""
experiments/privacy_analysis.py — 实验三：隐私预算 ε vs 性能

修复：
  - 删除子图大标题（set_title 改为仅保留子图字母标注）
  - 图例移至右上角
...
def run_privacy_analysis(args, dataset: str, build_config_fn,
    ...
    Train once at No-DP (ε=∞) to fix the CVAE backbone, then for each ε only
    ...
    logger and logger.info("  [Privacy] Training fixed backbone at No-DP (ε=∞) ...")
    args.epsilon  = 1e6
    ...
    # Sweep ε with frozen CVAE backbone
    for eps, label in zip(EPSILON_GRID[:-1], EPSILON_LABELS[:-1]):
        logger and logger.info(f"  [Privacy] ε = {label} (fixed backbone) ...")
        try:
            result = backbone.retrain_with_epsilon(eps)
            all_results[label] = result.get("FedPOT", {})
        except Exception as e:
            logger and logger.info(f"  [Privacy] ε={label} FAILED: {e}")
            all_results[label] = {}
...
def _draw_panel(ax, results: Dict):
    ...
    if not np.isnan(accs[default_x]) and not np.isnan(no_dp_acc):
        ...
        # When default ε gives the same or better Acc than No-DP (can happen
        ...
        if no_dp_acc > 0 and accs[default_x] <= no_dp_acc + 1e-4:
            ...
            annot_txt = f"ε={default_label}: {accs[default_x]:.3f}"
        ax.annotate(
            annot_txt,
    ...
    ax.set_xlabel("Privacy Budget ε", fontsize=FS_LABEL)
...
def plot_privacy_curve_combined(results_oc: Dict, results_cwru: Dict,
    ...
    # 左右子图分别加数据集标签（右上角小字）
    for ax, label in zip(axes, ["Office-Caltech10", "CWRU Bearing Fault"]):
...
def print_privacy_table(results: Dict, logger=None):
    sep   = "=" * 60
    lines = [sep,
             f"  {'ε':<14}  {'Accuracy':>10}  {'Macro-F1':>10}  {'AUC':>10}",
             "-" * 60]
    for label in EPSILON_LABELS:
        m      = results.get(label, {})
